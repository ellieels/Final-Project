...
export const changePassword = (req, res) => {
  res.render('changePassword', {customer: req.customer});
}
...
export const updatePassword = async (req, res) => {
  try {
    ...
    const cusomter = await Customer.findOne({ username });
    if (!customer) {
    ...
    customer.comparePassword(currPassword, async (err, isMatch) => {
      if (err) {
      ...
      customer.password = newPassword1; // This will be hashed automatically before saving due to the pre-save middleware
      await customer.save();
